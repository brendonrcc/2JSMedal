 // Global State
    ...
    async function attemptAutoLogin() {
        ...
        try {
            ...
            const user = verifyUserInTSV(username, tsvData);
    ...
    function verifyUserInTSV(searchNick, tsvText) {
        const rows = tsvText.split('\n').map(r => r.split('\t'));
        const search = searchNick.toLowerCase();
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (row.length > 15) {
                ...
                if (nickP === search) {
                    return { nick: cleanCell(row[15]), role: cleanCell(row[14]) };
                }
            }
        }
        return null;
    ...
    function showToast(message, type = 'info', title = '') {
         ...
         toast.innerHTML = `
             ...
             <div class="toast-content">
                 <span class="font-bold text-sm block">${finalTitle}</span>
    ...
    async function loadLeadershipData() {
         ...
         grid.innerHTML = '<div class="col-span-full text-center py-12"><i class="fas fa-circle-notch fa-spin text-4xl text-[#68a7b9] mb-3"></i><p class="text-gray-500 font-bold animate-pulse">Consultando...</p></div>';
         ...
         try {
             ...
             if(leaders.length === 0) {
                 grid.innerHTML = '<div class="col-span-full text-center text-gray-500 p-8 border border-dashed border-gray-700 rounded-xl">Nenhum membro da liderança encontrado.</div>';
                 return;
             }
             ...
             leaders.forEach(([nickKey, data]) => {
                 ...
                 html += `
                 <div class="group relative bg-[#161b22] rounded-2xl border border-white/5 hover:border-[#68a7b9]/50 transition-all duration-300 overflow-hidden cursor-pointer hover:-translate-y-1" onclick="openLeaderDetails('${displayNick}', '${data.cargo}', '${month}', '${year}')">
                    
                    <div class="h-24 bg-gradient-to-b from-[#1c2128] to-[#161b22] relative">
                         <div class="absolute inset-0 opacity-10" style="background-image: radial-gradient(#68a7b9 1px, transparent 1px); background-size: 10px 10px;"></div>
                    </div>
                    
                    <div class="px-4 pb-6 -mt-12 relative z-10 flex flex-col items-center">
                         <div class="w-20 h-20 p-1 bg-[#161b22] rounded-2xl shadow-sm rotate-3 group-hover:rotate-0 transition-transform duration-300 ring-1 ring-white/5 group-hover:ring-[#68a7b9]/30">
                             <img src="${avatar}" class="w-full h-full object-cover rounded-xl bg-[#0d1116]" onerror="this.src='https://i.imgur.com/S1tKqgc.gif'">
                         </div>
                         
                         <div class="mt-4 text-center w-full">
                             <h3 class="text-white font-bold text-lg tracking-tight group-hover:text-[#68a7b9] transition-colors">${displayNick}</h3>
                             <span class="inline-block mt-2 px-3 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-widest bg-[#21262d] text-[#8b949e] border border-white/5 group-hover:border-[#68a7b9]/30 group-hover:text-white transition-all">
                                ${data.cargo}
                             </span>
                         </div>
                         
                         <div class="mt-4 w-full flex justify-center border-t border-white/5 pt-4 opacity-50 group-hover:opacity-100 transition-opacity">
                            <span class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Clique para detalhes</span>
                         </div>
                    </div>
                 </div>`;
             });
    ...
    async function openLeaderDetails(nick, role, month, year) {
         ...
         descEl.innerHTML = `
             <div id="leadership-calc-container" class="font-sans">
                <div class="flex gap-2 justify-center mb-4 bg-black/20 p-1 rounded-lg w-fit mx-auto">
                     <button class="px-3 py-1 text-xs rounded-md bg-[#68a7b9] text-black font-bold" onclick="setLeaderCalcMode('auto')" id="opt-auto">AUTO</button>
                     <button class="px-3 py-1 text-xs rounded-md text-gray-400 font-bold hover:text-white" onclick="setLeaderCalcMode('manual')" id="opt-manual">MANUAL</button>
                </div>
 
                 <div id="manual-inputs" class="hidden manual-inputs-container bg-[#21262d] p-4 rounded-xl border border-white/5 mb-4">
                      <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-2">Inserção Manual</p>
                      <div class="grid grid-cols-2 gap-3">
                          <div>
                              <label class="text-[10px] text-gray-500 block mb-1">Início</label>
                              <input type="date" id="manual-start-date" class="form-input text-xs py-1" onchange="recalculateLeadership()">
                          </div>
                          <div>
                              <label class="text-[10px] text-gray-500 block mb-1">Retorno</label>
         ...
         window.setLeaderCalcMode = (mode) => {
             ...
             if(mode === 'auto') {
                 optAuto.style.backgroundColor = '#68a7b9'; optAuto.style.color = '#000';
                 optManual.style.backgroundColor = 'transparent'; optManual.style.color = '#9ca3af';
                 manualInputs.classList.add('hidden');
             } else {
                 optManual.style.backgroundColor = '#68a7b9'; optManual.style.color = '#000';
                 optAuto.style.backgroundColor = 'transparent'; optAuto.style.color = '#9ca3af';
         ...
         window.recalculateLeadership = () => {
             ...
             let leaveInfo = `<span class="opacity-60 text-xs">Nenhuma ausência registrada.</span>`;
             ...
             if (leaves.length > 0) {
                 ...
                 if (activeDaysCount < daysInMonth) {
                     statusColor = returnDate ? '#58a6ff' : '#d29922';
                     statusTitle = returnDate ? 'Retorno' : 'Em Licença';
                     ...
                     <div class="flex flex-col gap-1 text-xs">
                         <div class="flex justify-between border-b border-white/5 pb-1">
                             ...
                             <span class="font-mono text-white">${effectiveAbsenceStart.toLocaleDateString('pt-BR')}</span>
                         </div>
                         ${returnDate ? `
                         <div class="flex justify-between">
                             <span class="text-blue-400">Retorno:</span>
                             <span class="font-mono text-white">${returnDate.toLocaleDateString('pt-BR')}</span>
                         </div>` : 
                         `<div class="text-[10px] text-yellow-500 font-bold uppercase tracking-wide text-center mt-1">Sem retorno no mês</div>`}
             ...
             resultsArea.innerHTML = `
                 <div class="flex items-center gap-4 mb-4 bg-[#0d1116] p-4 rounded-xl border border-white/5 relative overflow-hidden">
                     <div class="absolute inset-0 bg-gradient-to-r from-transparent to-[#161b22]/50 pointer-events-none"></div>
                     <div class="shrink-0 z-10">
                         <img src="${avatarUrl}" class="w-14 h-14 object-contain drop-shadow-md bg-[#161b22] rounded-lg p-1">
                     </div>
                     <div class="flex-1 relative z-10">
                         <h4 class="text-lg font-bold text-white leading-tight">${nick}</h4>
                         <div class="flex items-center gap-2 mt-1">
                             <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-[#21262d] text-gray-400 border border-white/5">
                                 ${role}
                             </span>
                             <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1" style="color: ${statusColor}; border-color: ${statusColor}40; background: ${statusColor}10">
                 ...
                 <div class="grid grid-cols-3 gap-3 mb-4">
                     <div class="p-3 bg-[#0d1116] border border-white/5 rounded-xl text-center flex flex-col justify-center">
                         <span class="text-[9px] uppercase font-bold text-gray-500 tracking-wider">Total</span>
                         <span class="text-lg font-bold text-white">${daysInMonth}</span>
                     </div>
 
                     <div class="p-3 bg-[#0d1116] border border-white/5 rounded-xl text-center flex flex-col justify-center">
                         <span class="text-[9px] uppercase font-bold text-red-400 tracking-wider opacity-70">Off</span>
                         <span class="text-lg font-bold text-red-400">${absentDaysCount}</span>
                     </div>
 
                     <div class="p-3 bg-[#68a7b9]/10 border border-[#68a7b9]/30 rounded-xl text-center flex flex-col justify-center">
                         <span class="text-[9px] uppercase font-bold text-[#68a7b9] tracking-wider">Ativos</span>
                         <span class="text-lg font-bold text-[#68a7b9]">${activeDaysCount}</span>
                     </div>
                 </div>
 
                 <div class="mb-5 flex flex-col gap-3">
                     <div class="rounded-xl border border-white/5 p-3 bg-[#21262d]">
                         <p class="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-2 border-b border-white/5 pb-1">
                             Detalhamento
                         </p>
                         ${leaveInfo}
                     </div>
                 ...
                 <div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-900 to-[#161b22] text-white p-4 border border-white/5 flex items-center justify-between">
                     <div class="relative z-10">
                         <p class="text-[9px] font-bold text-gray-500 uppercase tracking-[0.2em] mb-1">Resultado Final</p>
                         <p class="text-xs text-gray-400 font-mono">Ref: ${fullMonthNames[monthIndex]}/${targetYear}</p>
                     </div>
                     <div class="flex flex-col items-end relative z-10">
                         <div class="flex items-baseline gap-1">
                             <span class="text-4xl font-extrabold text-[#e3b341]">
                                 ${finalMedals}
                             </span>
                         </div>
                         <span class="text-[9px] font-bold text-[#e3b341] uppercase tracking-widest">Medalhas</span>
                     </div>
                 </div>
             `;
    ...
    function renderGeneralResults() {
         ...
         if(hasFilter) {
             ...
             statusPos.innerHTML = `<span class="font-bold text-green-400">${pendingPos.length} aceito(s)</span> | <span class="text-gray-500">${rejectedPosCount} ignorado(s)</span>`;
             
             statusNeg.classList.remove('hidden');
             statusNeg.innerHTML = `<span class="font-bold text-red-400">${pendingNeg.length} aceito(s)</span> | <span class="text-gray-500">${rejectedNegCount} ignorado(s)</span>`;
         } else {
    ...
    function renderSkeleton(colsCount = 5, rowsCount = 8) {
         ...
         for(let i=0; i<rowsCount; i++) {
             let cells = '';
             for(let j=0; j<colsCount; j++) cells += `<td><div class="h-4 bg-[#21262d] rounded w-full"></div></td>`;
             html += `<tr><td class="col-avatar"><div class="w-9 h-9 bg-[#21262d] rounded-lg"></div></td><td class="col-nick"><div class="h-4 bg-[#21262d] w-3/4 rounded"></div></td>${cells}<td class="col-total"><div class="h-5 w-8 bg-[#21262d] rounded"></div></td><td><div class="h-6 w-16 bg-[#21262d] rounded-full"></div></td><td><div class="h-8 w-20 bg-[#21262d] rounded"></div></td></tr>`;
         }
 ...
 `;
    ...
    function renderMinistryResults() {
         ...
         if(statusFilter === 'Positivo') {
             resMinisters.className = "text-sm font-bold text-green-400 font-mono break-all min-h-[50px] flex items-center bg-[#0d1116] p-3 rounded-lg border border-white/5";
             resInterns.className = "text-sm font-bold text-green-400 font-mono break-all min-h-[50px] flex items-center bg-[#0d1116] p-3 rounded-lg border border-white/5";
             ...
             resMinisters.className = "text-sm font-bold text-red-400 font-mono break-all min-h-[50px] flex items-center bg-[#0d1116] p-3 rounded-lg border border-white/5";
             resInterns.className = "text-sm font-bold text-red-400 font-mono break-all min-h-[50px] flex items-center bg-[#0d1116] p-3 rounded-lg border border-white/5";
